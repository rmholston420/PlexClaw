...
def _validate_command(command: str) -> str:
    """Validate *command* or raise HTTP 400, returning the original token."""
    cmd = command.strip()
    if not cmd:
        raise HTTPException(status_code=400, detail="command must not be empty")

    shell_metacharacters = set(";|&$`(){}<>\n\r\t")
    if any(ch.isspace() for ch in cmd) or any(ch in shell_metacharacters for ch in cmd):
        raise HTTPException(
            status_code=400,
            detail=(
    ...
    if resolved is None:
        ...
        if p.is_absolute() and p.is_file():
            ...
            raise HTTPException(
                status_code=400,
                detail=(
                    f"command '{cmd}' not found on PATH and is not an existing "
                    "absolute path"
                ),
            )
...
async def add_mcp_server(req: MCPServerCreate) -> dict:
    ...
    safe_command = _validate_command(req.command)
    ...
    def _write() -> dict:
        ...
        return {
            "ok": True,
            "name": name,
            "resolved_command": shutil.which(safe_command) or safe_command,
        }
...
async def update_mcp_server(name: str, req: MCPServerUpdate) -> dict:
    ...
    def _patch() -> dict:
        ...
        if req.command is not None:
            srv["command"] = _validate_command(req.command)
